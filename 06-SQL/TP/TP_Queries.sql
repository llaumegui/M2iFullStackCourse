...
SELECT p.first_name, COUNT(*) AS nbr_dogs
FROM [Chien] c
...
SELECT p.first_name, SUM(c.weight) AS dog_weights
FROM [Chien] c
    INNER JOIN [Personne] AS p
    ON p.id = c.owner_id
GROUP BY p.first_name
...
somme de leur age respectives (age du maître et des chiens).*/
SELECT p.first_name, (p.age + SUM(c.age)) AS total
FROM [Chien] c
    INNER JOIN [Personne] AS p
    ON p.id = c.owner_id
